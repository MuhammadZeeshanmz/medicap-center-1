              ...
              <div class="tabs-navigation text-center">
                <ul class="nav nav-tabs d-inline-flex flex-wrap justify-content-center p-3 radius-md bg-light" style="gap: 10px;">
                  @foreach ($terms as $term)
                    <li class="nav-item">
                      <button class="nav-link btn-md radius-sm {{ $loop->iteration == ceil($loop->count / 2) ? 'active' : '' }}"
                              data-bs-toggle="tab"
                              data-bs-target="#{{ $term }}"
                              type="button">
                        {{ __($term) }}
            ...
            @foreach ($terms as $term)
              @php
                $packages = \App\Models\Package::where('status', '1')
                    ...
                    ->get();
              @endphp
              <div class="tab-pane fade {{ $loop->iteration == ceil($loop->count / 2) ? 'show active' : '' }}" id="{{ $term }}">
                <div class="row justify-content-center">
                  @foreach ($packages as $package)
                    <div class="col-md-6 col-lg-4 mb-4">
                      <div class="card pricing-card shadow-sm h-100 rounded-4 {{ $package->recommended == 1 ? 'recommended-card text-white' : 'bg-white' }}">
                        <div class="card-body d-flex flex-column align-items-center text-center">
                          <div class="d-flex align-items-center justify-content-center gap-2 mb-3">
  @if(!empty($package->image) && file_exists(public_path('assets/img/packages/' . $package->image)))
    <img src="{{ asset('assets/img/packages/' . $package->image) }}" alt="{{ $package->title }}" class="img-fluid package-image hover-rotate" style="height: 24px;">
  @else
    <i class="fas fa-box {{ $package->recommended == 1 ? 'text-white' : 'text-primary' }}"></i>
  @endif
  <h4 class="fw-bold mb-0">{{ $package->title }}</h4>
  @if ($package->recommended == 1)
                            <span class="badge bg-white text-danger mb-3">{{ __('Recommended') }}</span>
                          @endif
</div>
                          ...
                          <h5 class="mb-2">
                            @if ($package->price == 0)
                              {{ __('Free') }}
                            @else
                              {{ format_price($package->price) }}<small class="{{ $package->recommended == 1 ? 'text-light' : 'text-muted' }}"> / {{ __(str_replace('ly', '', $package->term)) }}</small>
                            @endif
                          </h5>
                          <ul class="list-unstyled text-start w-100 mt-3">
                            <li class="mb-2"><i class="fas fa-check me-2 text-success"></i>{{ __('Services') }} ({{ $package->number_of_service_add === 999999 ? __('Unlimited') : $package->number_of_service_add }})</li>
                            <li class="mb-2"><i class="fas fa-check me-2 text-success"></i>{{ __('Images/Service') }} ({{ $package->number_of_service_image === 999999 ? __('Unlimited') : $package->number_of_service_image }})</li>
                            <li class="mb-2"><i class="fas fa-check me-2 text-success"></i>{{ __('Appointments') }} ({{ $package->number_of_appointment === 999999 ? __('Unlimited') : $package->number_of_appointment }})</li>
                            <li class="mb-2"><i class="fas fa-check me-2 text-success"></i>{{ __('Staffs') }} ({{ $package->staff_limit === 999999 ? __('Unlimited') : $package->staff_limit }})</li>
                            <li class="mb-2">
                            ...
                            @if (!empty($package->custom_features))
                              ...
                              @foreach ($features as $value)
                                @if(trim($value))
                                  <li class="mb-2"><i class="fas fa-check me-2 text-success"></i>{{ trim($value) }}</li>
                                @endif
                              @endforeach
                            @endif
                          </ul>
